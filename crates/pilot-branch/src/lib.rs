...
// ─────────────────────────────────────────────────────────────────────────────
// P4 Deliverable 3: Branch Timeline
// ─────────────────────────────────────────────────────────────────────────────
...
fn timeline_path() -> PathBuf {
    let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
    PathBuf::from(home).join(".pilot").join("branch_timeline.jsonl")
}

/// Emit a structured branch timeline event.
pub fn emit_branch_event(event: &BranchTimelineEvent) -> Result<PathBuf> {
    let path = timeline_path();
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
    let line = serde_json::to_string(event)
        .map_err(|e| anyhow::anyhow!("Failed to serialize timeline event: {}", e))?;
    use std::io::Write;
    let mut file = std::fs::OpenOptions::new()
        .create(true)
        .append(true)
...
/// Query the branch timeline with filters and pagination.
pub fn query_branch_timeline(
    scope_id: Option<String>,
    action_filter: Option<&str>,
    limit: usize,
    offset: usize,
) -> Vec<BranchTimelineEvent> {
    let path = timeline_path();
    let content = match std::fs::read_to_string(&path) {
        Ok(c) => c,
        Err(_) => return vec![],
    ...
    let mut events: Vec<BranchTimelineEvent> = content
        .lines()
        .filter_map(|line| serde_json::from_str(line).ok())
        .filter(|e: &BranchTimelineEvent| {
            let scope_ok = scope_id.as_deref().map_or(true, |sid| e.scope_id.as_deref() == Some(sid));
